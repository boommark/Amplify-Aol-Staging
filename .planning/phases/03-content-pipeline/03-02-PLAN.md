...
<task type="auto">
  ...
  <read_first>
    ...
    - lib/db/research.ts (created by Plan 01 — use saveResearchDimension for persistence)
...
IMPORTANT: Use `saveResearchDimension` from `@/lib/db/research` for persistence — do NOT call `supabase.from('campaign_research').insert()` directly. The DB utility layer (Plan 01) owns all campaign_research writes.
...
Note: This route returns JSON (not streaming) because each dimension resolves as a complete unit. The chat route (Plan 05) will call this pipeline and stream results as data parts to the client.
  ...
  <acceptance_criteria>
    ...
    - lib/pipeline/research.ts imports `saveResearchDimension` from `@/lib/db/research` and uses it for persistence (NOT direct supabase.from calls for inserts)
  ...
  <done>Research pipeline fires 7 Perplexity queries in parallel, persists each dimension via saveResearchDimension from lib/db/research, supports adding user notes, and packages research into context string for downstream use</done>
</task>
