...
export default async function AppLayout({
  ...
  // Shared sidebar content rendered in both desktop and mobile Sheet
  const sidebarContent = (
    <>
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <a
          href="/chat"
          className="flex items-center gap-2 w-full px-4 py-2.5 rounded-lg text-white text-sm font-medium transition-colors cursor-pointer"
          style={{ backgroundColor: '#3D8BE8', fontFamily: 'Work Sans, sans-serif' }}
        >
        ...
        {/* Past campaigns list grouped by recency */}
        <div className="pt-4">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 mb-2" style={{ fontFamily: 'Work Sans, sans-serif' }}>
            Recent Campaigns
          </p>
          <CampaignList />
        </div>
      ...
      <div className="p-4 border-t border-gray-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm text-gray-700 truncate" style={{ fontFamily: 'Work Sans, sans-serif' }}>{displayName}</span>
          </div>
          <form action={handleSignOut}>
            <button
              type="submit"
              className="text-xs text-gray-400 hover:text-gray-600 transition-colors ml-2 cursor-pointer"
              title="Sign out"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        <Sheet>
          <SheetTrigger asChild>
            <button
              className="p-1 rounded-md text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-colors cursor-pointer"
        ...
        <div
          ...
          style={{ backgroundColor: '#3D8BE8' }}
        >
          <span className="text-white font-bold text-sm tracking-wide" style={{ fontFamily: 'Raleway, sans-serif' }}>
            Amplify
          </span>
        </div>

        <a href="/chat" className="text-sm font-medium cursor-pointer" style={{ color: '#3D8BE8', fontFamily: 'Work Sans, sans-serif' }}>
          New
        </a>
      </div>
